...
class SalesEngine
  ...
  def initialize(items_table=nil, merchants_table=nil, invoice_table=nil,
    ...
    @transactions = TransactionRepository.new(transactions_table, self) if transactions_table
    @customers = CustomerRepository.new(customers_table, self) if customers_table
  end
  ...
  def self.from_csv(file_hash)
    ...
    transactions_table = load_data(file_hash[:transactions]) if file_hash[:transactions]
    customers_table = load_data(file_hash[:customers]) if file_hash[:customers]
